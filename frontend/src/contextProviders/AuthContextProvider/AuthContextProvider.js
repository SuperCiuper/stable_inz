    ...
    fetch(API_URL + "auth/login", {
      ...
      .catch((err) => {
        ...
        toast.current.show({
          severity: "error",
          summary: "Błąd",
          detail: "Próba logowania nie powiodła się",
          life: 10000,
        });
      });
  };
  ...
  const performDataUpdate = (url, method, body, callback) => {
    ...
    if (!authToken) {
      toast.current.show({
        severity: "error",
        summary: "Token lost",
        detail: "Nie znaleziono tokena, wylogowano!",
        life: 10000,
      });
      logoutUser();
      return;
      ...
      .catch((err) => {
        ...
        if (err === "No token provided!") {
          toast.current.show({
            severity: "error",
            summary: "Błąd",
            detail: `"${err}", token zgubiony, zostałeś wylogowany!`,
            life: 6000,
          });
          logoutUser();
          return;
        ...
        if (err === "Unauthorized!") {
          toast.current.show({
            ...
            detail: "Nie masz uprawnień, aby modyfikować ten zasób!",
            life: 6000,
          });
          return;
        }
        toast.current.show({
          severity: "error",
          summary: "Błąd",
          detail: `Błąd serwera: "${err}", zmiany nie zostały zapisane`,
          life: 6000,
        });
      });
  };
    ...
    if (authContext.isLogged) {
      ...
      if (!expirationTime || Date.now() >= expirationTime) {
        toast.current.show({
          severity: "warn",
          summary: "Wylogowano",
          detail: "Przekroczony czas sesji, wylogowano!",
          life: 10000,
        });
        logoutUser();
        return;
