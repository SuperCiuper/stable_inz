		...
		<div className='ApiaryView'>
			...
			<div className='ApiaryTableContainer'>
				<table className='ApiaryTable'>
					...
					<tbody>
						{apiaries
							.sort((a, b) => (sortedDown ? a.number - b.number : b.number - a.number))
							.filter((item) => {
								if (filterDates.from !== "") {
									if (item.date < filterDates.from.split("-").reverse().join(".")) return false;
								}
								if (filterDates.to !== "") {
									if (filterDates.to.split("-").reverse().join(".") < item.date) return false;
								}
								return true;
							})
							.map((item) => (
			...
			<AddApiaryModal apiariesRefresh={fetchApiaries} visibilityToggle={visibilityToggle} visible={modalVisible} />
		</div>
	);
};
