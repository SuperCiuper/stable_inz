...
const TrainerView = () => {
	...
	const addNewTrainer = (newTrainer, newImages) => {
		...
		authContext.performDataUpdate("trainer", "POST", { name: newTrainer.name, description: newTrainer.description, image: image }, fetchTrainerList);
	};
	...
	const setNewTrainerDescription = (newTrainer, description) => {
		...
		openImageSelector(`Wybierz profilowe trenera ${newTrainer.name}`, [], (images) => addNewTrainer({ ...newTrainer, description: description }, images), true);
	};
		...
		<div className='trainer-view'>
			...
			{authContext.isLogged ? (
				<div className='new-trainer'>
					<Toast ref={toast} />
					<Button className='add-trainer-btn p-button-sm p-button-secondary' onClick={createNewTrainer} label='Dodaj nowego trenera' />
				</div>
			) : (
				""
			)}
