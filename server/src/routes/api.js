...
router.put("/colors", (req, res) => {
  ...
  if (
    ...
    return res.status(400).json("Mandatory fields not set");
...
router.delete("/textBlocks", (req, res) => {
  if (!req.body || !req.body.id) return res.status(400).json("Id not provided");
...
router.delete("/horses", (req, res) => {
  if (!req.body || !req.body.name) return res.status(400).json("Name not provided");
...
router.post("/prices", (req, res) => {
  ...
  if (!newPrice || !newPrice.name || !newPrice.price) return res.status(400).json("Mandatory fields not set");
...
router.post("/images", (req, res) => {
  if (!req.files || !req.files.images) return res.status(400).json("No images sent");
  let images = req.files.images;

  if (!Array.isArray(images)) images = [images];
  else if (images.length === 0) return res.status(400).json("Image list not sent");
...
router.put("/images", (req, res) => {
  let images = req.body;
  if (!images || images.length === 0) return res.status(400).json("Image list not sent");

  const imageList = databaseConnector.getImageList();
  for (const image of images) {
    if (!image.image || !(typeof image.visible === "boolean"))
      return res.status(400).json("One of updated images lacks mandatory fields");
...
router.delete("/images", (req, res) => {
  ...
  if (!deleteImageNames || deleteImageNames.length === 0) return res.status(400).json("No images sent");
